...
Axiom Retention Group — core churn prediction pipeline.

Expects clean, standardised data matching the schema produced by create_data.py
(or any real client export that has been pre-processed to that schema).

Produces:
  - A trained, best-selected model
  - The Axiom watchlist (members above the risk threshold)
  - A JSON report payload ready for automated report generation

Usage:
    python pipeline.py --data data/members.csv
    python pipeline.py --data data/members.csv --threshold 70 --save-rate 0.30
    python pipeline.py --data data/client_x.csv --fee-col monthly_fee --out outputs/client_x.json
"""
